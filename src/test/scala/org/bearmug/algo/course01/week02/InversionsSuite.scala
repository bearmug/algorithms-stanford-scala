...
class InversionsSuite extends FunSuite {
  ...
  test("inversionsSimple works for 100000 inversed elements") {
    assert(Inversions((1 to 100000 reverse).toList).inversionsSimple() == (100000 / 2) * (100000 - 1))
  }

  test("inversionsPar works for 100000 inversed elements") {
    assert(Inversions((1 to 100000 reverse).toList).inversionsPar(4) == (100000 / 2) * (100000 - 1))
  }
}
